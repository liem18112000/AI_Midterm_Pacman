...
def aStarSearch(problem, heuristic=nullHeuristic, maxThreshold=999999):
    ...
    pqueue.push((problem.getStartState(), [], 0), heuristic(
        problem.getStartState(), problem) + 0)
    ...
    while not pqueue.isEmpty():
        ...
        for successor in successors:
            ...
            if childVertex not in visited:
                ...
                pqueue.push((childVertex, fullPath, totalCost),
                            totalCost + heuristic(childVertex, problem))

            else:

                # Update (Node, [Path], Culmulative backward cost) in the pqueue.
                pqueue.update((childVertex, fullPath, totalCost),
                              totalCost + heuristic(childVertex, problem))
